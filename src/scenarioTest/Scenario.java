	...
	public static void main(String[] args) {
		...
		IVillage village = new IVillage() {
			...
			public DepenseMarchand[] acheterProduit(String produit, int quantiteSouhaitee) {
				...
				int nbDepenses = 0;
				for (int i = 0; i < nbEtal && quantiteReste > 0; i++) { // parcours vendeurs etals tant que > 0
					int quantiteVendre = marche[i].contientProduit(produit, quantiteReste); //q restante
					if (quantiteVendre != 0 ) {
						quantiteReste -= quantiteVendre;
						double prixPaye = marche[i].acheterProduit(quantiteVendre); // prix de transaction
						depenses[nbDepenses] = new DepenseMarchand(marche[i].getVendeur(), quantiteVendre, produit, prixPaye);
						nbDepenses++;
						// new objet DepenseMarchand -> dépenses faites auprès du vendeur actuel stockage dans le tableau depenses.
					}
				}
